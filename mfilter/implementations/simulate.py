...
class SimulateTimes:
    ...
    def _get_epsilon(self):
        ...
        return self.epsilon[self.counter_epsilon-1]
    ...
    def _compute_delta(self, underlying_delta):
        ...
        return round(underlying_delta, 2)
    ...
    def _add_irregularities(self):
        for i in range(self.n_samples):
            self.times[i] += self._get_epsilon()
    ...
    def get_times(self, configuration=1, time_init=None):
...
class SimulateSignal(SimulateTimes):
    def __init__(self, n_samples, freq, weights=None, underlying_delta=None,
                 small_sigma=None, seed=None, noise_level=None):
    ...
    def _compute_freqs(self, freq, weights):
        if not isinstance(freq, (np.ndarray, list)):
            ...
            if weights.ndim != 1 or freq.ndim != 1:
    ...
    def _get_range_of_signal(self, pos_start_peaks, n_peaks):
        ...
        if self.freqs is None:
            self.freqs = [0.5 / (max(self.times) - time_start)]
            idx_end = len(self.times)-1
        else:
            max_period = min(self.freqs)
    ...
    def get_data(self, pos_start_peaks=None, n_peaks=None,
                 seed=None, with_noise=False):

        _ = self.get_times(configuration=1, time_init=0)
        idx_start, idx_end = self._get_range_of_signal(pos_start_peaks,
                                                       n_peaks)
        time_duration = self.times[idx_end] - self.times[idx_start]
        isolated_time = np.linspace(0, time_duration, idx_end - idx_start)
        # create the vector
        data = np.zeros(self.n_samples)
        for i in range(len(self.freqs)):
            data[idx_start:idx_end] += self._wave(i, isolated_time)
